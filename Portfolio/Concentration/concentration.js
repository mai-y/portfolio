...
function openCard(event) {
    ...
    if(card2 != null || event.target.className == "card matched" || event.target.className == "")return;
...
function keepCard(){
    ...
    if(matched.length == places.length){
        ...
        if(bestRecord == null || bestRecord > newRecord){
            ...
            saveDate("BestRecord",bestRecord);
...
function getTimeString(value){
    ...
    var ms = fixDigits(Math.floor(date.getMilliseconds()/10));
    return m + ":" + s + "." + ms;
}
...
function setBackground(event){
    ...
    image.onload = function(){
        ...
        saveDate("PanelImage",canvas.toDateURL());
    }
    images.src = url;
}
...
function hideControlPanel(event){
    ...
    document.addEventListener(moveEment,showControlPanel);
}
...
function showControlPanel(event){
    ...
    document.removeEventListener(moveEment,showControlPanel);
}
